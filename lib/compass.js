...
compass.compile = function(filePath, opts, callback) {
  ...
  if (opts && opts._showCommand) {
    console.log(commandOptins.join(' '))
  }
  ...
  child.on('close', function(code) {
    ...
    fileList.forEach(function(i) {
      ...
      if (!opts.relative) { // 绝对地址
        var form = path.basename(opts.srcDir);
        var reg = new RegExp('/' + form + '/', 'g');
        var to = path.basename(opts.buildDir);
        if (opts.ignoreBuildDir) {
          contents = String(contents).replace(reg, '/');
        } else {
